...
Je démarre la VM en headless désormais avec le script `VBoxHeadless -s maVM -v on`. Je ne garde que les services essentiels trouvés avec `sudo service --status-all` et je coupe les autres avec `sudo systemctl stop monservice`, je les disable ensuite avec `sudo update-rc.d cups disable`.
